...
from yarely.frontend.core.helpers.zmq import ZMQ_ADDRESS_INPROC, \
        ...
        ZMQ_SUBSMANAGER_REP_PORT, ZMQ_SUBSMANAGER_REQ_PORT
...
from yarely.frontend.core.subscriptions.subscription_manager.persistence \
    ...
    def _handle_request_subscription_update(self, msg_root, msg_elem):
        ...
        with self._lock:
            ...
            for child in content_descriptor_set.get_children():
                if (isinstance(child, ContentDescriptorSet) and
                    ...
                    for source in sources:
                        if (self._lookup_executing_handler_with_uri(
                            source.get_uri()) is not None):
                            is_handled = True
                            break
    ...
    def _init_handlers(self):
        ...
        file_handler = HandlerStub(
            command_line_args=python_launch_str.format(
                module='yarely.frontend.core.subscriptions.handlers.file'
            )
        )
        http_handler = HandlerStub(
            command_line_args=python_launch_str.format(
                module='yarely.frontend.core.subscriptions.handlers.http'
            )
        )
