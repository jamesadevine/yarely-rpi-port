...
from yarely.frontend.core.helpers.zmq import ZMQ_REQUEST_TIMEOUT_MSEC
from yarely.frontend.core.helpers.zmq import ZMQ_SOCKET_LINGER_MSEC
from yarely.frontend.core.helpers.zmq import ZMQ_SOCKET_NO_LINGER
from yarely.frontend.core.helpers.base_classes import \
ApplicationWithBasicLogging
from yarely.frontend.core.helpers.base_classes import ApplicationError
from yarely.frontend.core.helpers.base_classes import ZMQRPC
...
class Handler(ApplicationWithBasicLogging, ZMQRPC):
    ...
    def _generate_register(self, registration_token):
        root = ElementTree.Element(
            tag='register', attrib={'token': registration_token}
        )
        return root

    def _generate_subscription_update(self, uri, subscription_xml):
        root = ElementTree.Element(
            tag='subscription_update', attrib={'uri': uri}
        )
    ...
    def check_in(self):
        """Provide an occasional check-in to the Manager via ZMQ."""
    ...
    def start(self, register=True):
        ...
        If register is True (default), register with the manager, otherwise
        the caller will need to call self.register() once ready.
