...
HEX_PREFIX_ERROR_MSG = 'Expected # prefix for rgb hex string of length \
{length} (supplied string was \'{value}\')'
...
class RGBColour:
    ...
    Understands the following formats:
        * a hex representation string of length 3 or 6 (optionally with a
          ...
          >>> RGBColour("F0F")      # doctest: +SKIP_ON_PY2
          RGBColour('#ff00ff')
          >>> RGBColour("#f0f")     # doctest: +SKIP_ON_PY2
          RGBColour('#ff00ff')
          >>> RGBColour("ff00ff")   # doctest: +SKIP_ON_PY2
          RGBColour('#ff00ff')
          >>> RGBColour("#ff00ff")  # doctest: +SKIP_ON_PY2
          RGBColour('#ff00ff')
        ...
        * an arithmetic triplet (values between 0 and 1).
          ...
          >>> RGBColour((1.0, 0.0, 1.0))    # doctest: +SKIP_ON_PY2
          RGBColour('#ff00ff')
    ...
    def as_digital_16bit_triplet(self):     # Should return a tuple
        """Not implemented."""              # of three 0-65535 int values
    ...
    def as_string(self, length=7):
        ...
        if length < 6:
            ...
            if True in [hex_val[0] != hex_val[1] for hex_val in hex_tuple]:
                msg = 'Loss of precision: value {orig} cannot be accurately \
represented in length {length}, value {new} will be used.'
                msg = msg.format(orig=hex_tuple, length=length,
                                 new=hex_truncated)
                warnings.warn(msg)
